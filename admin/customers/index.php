...
<script>
$(function () {
    $('#import-customers-form').on('submit', function (event) {
        event.preventDefault();
        var form = this;
        var button = $(form).find('button[type="submit"]');
        var feedback = $('#customer-import-feedback');
        button.prop('disabled', true).text('Importando...');
        feedback.attr('class', 'customer-import-feedback').text('');
        $.ajax({
            url: _base_url_ + 'class/Customer.php?action=import_customers',
            method: 'POST',
            ...
            var details = response.errors && response.errors.length ? ' ' + response.errors.join(' ') : '';
            feedback.attr('class', 'customer-import-feedback ' + (response.status === 'success' ? 'success' : 'error'))
                .text((response.msg || 'Não foi possível importar os clientes.') + details);
            if (response.status === 'success' && response.created > 0) {
                window.setTimeout(function () { window.location.reload(); }, 1400);
            }
        }).fail(function (request) {
            var message = request.responseJSON && request.responseJSON.msg
                ? request.responseJSON.msg
                : 'O servidor não conseguiu processar o CSV. Confira o formato e tente novamente.';
            feedback.attr('class', 'customer-import-feedback error').text(message);
        }).always(function () {
            button.prop('disabled', false).text('Importar CSV');
        });
    });
});
</script>

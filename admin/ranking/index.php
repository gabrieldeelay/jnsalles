...
.ranking-datetime-control{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:7px}.ranking-now-button{display:inline-flex;min-width:68px;min-height:44px;align-items:center;justify-content:center;gap:5px;padding:0 11px;border:1px solid #475569;border-radius:9px;background:#172033;color:#ddd6fe;font-size:11px;font-weight:800;white-space:nowrap;cursor:pointer;transition:border-color .17s,background-color .17s,color .17s}.ranking-now-button:hover,.ranking-now-button:focus-visible{border-color:#8b5cf6;background:#26203f;color:#fff;outline:none;box-shadow:0 0 0 3px rgba(139,92,246,.14)}.ranking-now-button i{font-size:13px}
    ...
    <section class="ranking-card ranking-timer-card">
      ...
      <form id="ranking-timer-form">
        <div class="ranking-timer-grid">
          ...
          <div class="ranking-field"><label for="timer_start_at">Início</label><div class="ranking-datetime-control"><input name="start_at" id="timer_start_at" type="datetime-local" value="<?= $escape($timerStartValue) ?>"><button type="button" class="ranking-now-button" data-fill-now="timer_start_at" aria-label="Preencher início com a data e hora atuais" title="Usar a data e hora atuais"><i class="bi bi-clock"></i><span>Agora</span></button></div></div>
          <div class="ranking-field"><label for="timer_end_at">Encerramento</label><div class="ranking-datetime-control"><input name="end_at" id="timer_end_at" type="datetime-local" value="<?= $escape($timerEndValue) ?>"><button type="button" class="ranking-now-button" data-fill-now="timer_end_at" aria-label="Preencher encerramento com a data e hora atuais" title="Usar a data e hora atuais"><i class="bi bi-clock"></i><span>Agora</span></button></div></div>
...
(function () {
  ...
  document.querySelectorAll('[data-fill-now]').forEach(function (button) {
    button.addEventListener('click', function () {
      var input = document.getElementById(button.dataset.fillNow);
      if (!input) return;
      input.value = currentBrasiliaDatetime();

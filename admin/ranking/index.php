...
$timerReset = $timerPrefix !== '' ? trim((string) $_settings->info($timerPrefix . 'reset')) : '';
$timerConfigured = $timerStart !== '' && $timerEnd !== '' && strtotime($timerStart) && strtotime($timerEnd) && strtotime($timerEnd) > strtotime($timerStart);
$timerPreservedSince = $timerConfigured && $timerReset !== '' && strtotime($timerReset) >= strtotime($timerStart) ? $timerReset : $timerStart;
...
<main class="h-full overflow-y-auto">
  <div class="container mx-auto ranking-shell">
    ...
    <section class="ranking-card ranking-timer-card">
      <header class="ranking-card-head"><span class="ranking-card-icon">⏱</span><div><h3>Período do Top Compradores Diário</h3><p>Estenda o período atual sem perder o ranking ou inicie um novo período quando quiser zerar a classificação.</p></div></header>
      <form id="ranking-timer-form">
        ...
        <p class="ranking-timer-help">Horário de Brasília. “Estender” mantém compradores, posições e cotas acumuladas. “Iniciar novo período” zera a classificação e começa uma nova contagem.</p>

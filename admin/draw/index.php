...
<main class="h-full overflow-y-auto">
    <div class="container mx-auto draw-shell">
        ...
        <section class="draw-card">
            ...
            <div class="draw-simulation-banner" id="draw-simulation-banner">⚠ Simulação educacional — resultado sem validade</div>
            <div class="draw-stage">
                ...
                <div class="draw-result" id="draw-result"><p class="draw-winner-label" id="draw-winner-label">Ganhador oficial</p><h3 id="draw-winner-name"></h3><p class="draw-result-phone" id="draw-winner-phone"></p><div class="draw-result-meta"><span>Cota: <strong id="draw-winning-number"></strong></span><span>Campanha: <strong id="draw-campaign-name"></strong></span><span>Participações: <strong id="draw-eligible-count"></strong></span></div><p class="draw-audit" id="draw-audit-hash"></p></div>
...
<div class="draw-modal-backdrop" id="draw-settings-modal" aria-hidden="true">
    <section class="draw-modal" role="dialog" aria-modal="true" aria-labelledby="draw-settings-title">
        ...
        <div class="draw-modal-body">
            <div class="draw-sim-warning">Modo exclusivamente educacional. Os nomes ficam somente neste navegador, não são associados a compradores reais, não alteram o banco e todo resultado simulado aparece marcado como “sem validade”.</div>
...
(function () {
    ...
    var simulationBanner = document.getElementById('draw-simulation-banner');
    ...
    function renderSimulationQueue() {
        ...
        simulationBanner.classList.toggle('active', queue.length > 0);
    ...
    function showOfficialResult(draw) {
        ...
        simulationBanner.classList.remove('active');
    ...
    function showSimulationResult(name) {
        revealBase();
        result.classList.add('simulation');
        winnerLabel.textContent = 'Simulação — sem validade';
        simulationBanner.classList.add('active');
        document.getElementById('draw-winner-name').textContent = name;
        document.getElementById('draw-winner-phone').textContent = '(00) 0**-0000**';
        document.getElementById('draw-winning-number').textContent = 'SIMULAÇÃO';
        document.getElementById('draw-campaign-name').textContent = product.options[product.selectedIndex].text;
        document.getElementById('draw-eligible-count').textContent = 'Não aplicado';
        document.getElementById('draw-audit-hash').textContent = 'SIMULAÇÃO EDUCACIONAL — RESULTADO NÃO REGISTRADO E SEM VALIDADE.';
    }
    ...
    form.addEventListener('submit', async function (event) {
        ...
        var queue = loadSimulationQueue();
        var simulatedName = queue.length ? queue.shift() : '';
        ...
        if (simulatedName) {
            ...
            try {
                ...
                showSimulationResult(simulatedName);
                setFeedback('success', 'Simulação concluída. O nome foi removido da fila e nenhum dado oficial foi alterado.');
            } catch (error) {
                resetAfterError();
    ...
    document.getElementById('draw-modal-save').addEventListener('click', function () {
        ...
        saveSimulationQueue(queue);
        setModal(false);
        setFeedback('success', queue.length ? 'Fila de simulação salva com ' + queue.length + ' nome(s).' : 'Fila de simulação limpa. O próximo sorteio será oficial.');
    });

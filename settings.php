...
{
error_log(">>> FUNÇÃO pay2m_generate_pix FOI CHAMADA <<<");
    global $_settings;
    global $conn;
    ...
    $curl = curl_init();
    // Debug completo do valor original
file_put_contents(__DIR__ . '/log_pay2m.txt', "VALOR ORIGINAL (bruto): " . $amount . "\n", FILE_APPEND);

// Força conversão e formatação do valor
$amount = (float) str_replace(',', '.', $amount);
$amount = number_format($amount, 2, '.', '');

// Debug do valor final que vai pra Pay2m
file_put_contents(__DIR__ . '/log_pay2m.txt', "VALOR FORMATADO (final): " . $amount . "\n", FILE_APPEND);
...
// DEBUG bruto do CPF antes de enviar
file_put_contents(__DIR__ . '/log_pay2m.txt', "CPF recebido: " . $client_cpf . "\n", FILE_APPEND);

// Garante que só envia CPF válido com 11 dígitos
if (!empty($client_cpf) && preg_match('/^[0-9]{11}$/', $client_cpf) && $client_cpf !== "00000000000") {
    $data['generator_document'] = $client_cpf;
    file_put_contents(__DIR__ . '/log_pay2m.txt', "CPF enviado: " . $client_cpf . "\n", FILE_APPEND);
} else {
    file_put_contents(__DIR__ . '/log_pay2m.txt', "CPF REMOVIDO do payload\n", FILE_APPEND);
}
    ...
    // Debug: Ver resposta crua da Pay2m
    file_put_contents(__DIR__ . '/log_pay2m.txt', "RESPONSE:\n" . $response . "\n\n", FILE_APPEND);
    
    $payment = json_decode($response, true);
    
    // Debug: Ver array decodificado
    file_put_contents(__DIR__ . '/log_pay2m.txt', "PARSED:\n" . print_r($payment, true) . "\n\n", FILE_APPEND);
...
function paggue_generate_pix($oid, $total_amount, $customer_name, $customer_email, $order_expiration)
{
    ...
    $order_id = $oid;
    $order_item = $order_id;
    $tax = $_settings->info('paggue_tax') ?? 0;
